...
func Seed(db *gorm.DB) {
	...
	product1 := models.Product{Name: "Product 1", CategoryID: 1, Price: 15000}
	product2 := models.Product{Name: "Product 2", CategoryID: 2, Price: 25000}

	db.Create(&product1)
	db.Create(&product2)

	transaction1 := models.Transaction{UserID: user1.ID, Amount: 30000}
	transaction2 := models.Transaction{UserID: user2.ID, Amount: 75000}

	db.Create(&transaction1)
	db.Create(&transaction2)

	item1 := models.TransactionItem{TransactionID: transaction1.ID, ProductID: product1.ID, Quantity: 2, Price: 15000}
	item2 := models.TransactionItem{TransactionID: transaction2.ID, ProductID: product2.ID, Quantity: 3, Price: 25000}
